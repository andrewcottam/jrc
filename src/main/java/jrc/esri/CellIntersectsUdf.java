...
/**
 * Returns the intersection geometry between a cell and a given geometry. Works
 * on WKB.
 */
public class CellIntersectsUdf extends UDF {
	...
	private final OperatorIntersection intersectionOperator = (OperatorIntersection) OperatorFactoryLocal.getInstance().getOperator(Operator.Type.Intersection);
	...
	public BytesWritable evaluate(double cellSize, long cell, BytesWritable b) {
		...
		Geometry geometry = intersectionOperator.execute(cellEnvelope, ogcGeometry.getEsriGeometry(), SPATIAL_REFERENCE, null);
		...
		result.set(buffer.array(), 0, buffer.limit()); // Note: ESRI does always
														// use absolute puts so
														// the position isn't
														// advanced!
		return result;
	}
